...
Nothing is hard-coded to Simtabi: the issuer prefix, the reserved aliases, and the morph map are all configuration, so another company can run the standard for itself by changing values.
...
The three-letter issuer prefix — Simtabi's equivalent of an IEEE OUI. It is stamped into the identifier-shape `CHECK` constraint at migration time, so changing it after identifiers exist requires a migration. `spec_edition` is stamped on every issue and is never mutated.
...
`SisMorphServiceProvider` boots first and calls `Relation::enforceMorphMap()`, so an unmapped morph is a **critical Eloquent failure**, never a silently stored string. With `record_in_database` on, allocations are also written to the append-only `sis_morph_aliases` table for audit and drift detection. See [attach a subject](recipes/attach-a-subject.md).
...
'aliases' => [
    'reserved' => [
        // Defaults to the core AliasPolicy §5.3 list when empty.
    ],
    'strategy' => DefaultAliasStrategy::class,
],
```

Reserved mnemonic aliases default to the specification's §5.3 list (`SIMT`, `PROS`, `TEST`, `NULL`, `VOID`, `TEMP`, `DEMO`, `NONE`, `ADMIN`, `ROOT`, `SYST`); a consumer may extend it. The derivation strategy is swappable. See [alias derivation](tools/aliases.md).

## Serials

```php
'serials' => [
    'default_width' => 6,          // 6–9 digits; widening is safe, narrowing is forbidden
    'start_overrides' => [
        // 'INV' => 1000,
    ],
],
...
Serial widths are 6 to 9 digits. Starts come from the class register (global classes start at `100001` so the sequence never advertises counts; scoped classes start at `1`); overrides here are rarely needed.
...
Registered from `SisScheduleServiceProvider`, never pasted into `routes/console.php`. Every entry is individually disableable.
