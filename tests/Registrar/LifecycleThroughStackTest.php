...
final class LifecycleThroughStackTest extends TestCase
{
    ...
    private function alias(string $value): Alias
    {
        return $this->app->make(SisEngine::class)->alias($value);
    }
    ...
    {
        $id = $this->reserveCommission($this->class(SimClass::PERSON), null);
    ...
    {
        $client = $this->reserveCommission($this->class(SimClass::CLIENT), null, $this->alias('ADIQ'), SubjectRef::of('client', '7'));
        ...
        $inv1 = $this->reserveCommission($this->class(SimClass::INVOICE), 'ADIQ');
        $inv2 = $this->reserveCommission($this->class(SimClass::INVOICE), 'ADIQ');
