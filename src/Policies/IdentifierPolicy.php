 ...
 * explicitly by SisServiceProvider; SIS never relies on Laravel's policy auto-discovery, because an
...
final class IdentifierPolicy
{
    ...
    private function check(?Authenticatable $user, SisAbility $ability, SisRecord $record): bool
    {
        ...
        return $this->resolver->allows($actor, $ability, new AuthorizationContext($identifier->class, $record->scope, $identifier));
    }
}
