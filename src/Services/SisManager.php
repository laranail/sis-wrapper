...
final class SisManager
{
    ...
    public function reserve(ClassDefinition|SimClass|string $class, ?string $scope = null, string $reason = '', ?Actor $actor = null, int $width = 6): Identifier
    {
        return ($this->reserve)(new ReserveData(
            class: $class instanceof ClassDefinition ? $class : $this->engine->class($class),
            scope: $scope,
            reason: $reason,
    ...
    public function mint(ClassDefinition|SimClass|string $class): Minter
    {
        return $this->engine->mint($class);
    }
    ...
    {
        return $this->engine->parse($value);
    }

    public function classOf(string $value): ?ClassDefinition
    {
        return $this->engine->identify($value);
    }

    public function aliasCandidates(string $legalName): AliasCandidates
    {
        return $this->engine->aliasCandidates($legalName);
    }

    public function version(string $value): Version
    {
        return $this->engine->version($value);
    }
